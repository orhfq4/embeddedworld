...
uint8_t SPI_master_init(volatile SPI_t *SPI_addr, uint32_t clock_rate)
{
  ...
  gpio_inst_t MOSI_pin, SCK_pin; // Temporary objects to initialize pins
  return_val=no_errors;
  
  if(SPI_addr==SPI0)
  {
	  // Set Port B pins for SPI
	  GPIO_output_ctor(&MOSI_pin,SPI0_MOSI_PORT,SPI0_MOSI_PIN,1);
	  
	  if(CPOL_select==CPOL_0)
	  {
		  GPIO_output_ctor(&SCK_pin,SPI0_SCK_PORT,SPI0_SCK_PIN,0);
	  }
	  else
	  {
		  GPIO_output_ctor(&SCK_pin,SPI0_SCK_PORT,SPI0_SCK_PIN,1);
	  }
  }
  else if(SPI_addr==SPI1)
  {
	  // Set
	  GPIO_output_ctor(&MOSI_pin,SPI1_MOSI_PORT,SPI1_MOSI_PIN,1);
	  if(CPOL_select==CPOL_0)
	  {
		  GPIO_output_ctor(&SCK_pin,SPI1_SCK_PORT,SPI1_SCK_PIN,0);
	  }
	  else
	  {
		  GPIO_output_ctor(&SCK_pin,SPI1_SCK_PORT,SPI1_SCK_PIN,1);
	  }
  }
  else
  {
