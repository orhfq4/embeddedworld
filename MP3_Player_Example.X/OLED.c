...
void OLED_reset_init(void)
{
	GPIO_output_ctor(&oled_reset, OLED_Reset_Port,OLED_Reset_Pin, GPIO_CLEAR);
}
...
***************************************************************/									
void OLED_reset_control(GPIO_PIN_STATE_t value)
{
	GPIO_output_set_value(&oled_reset,value);
}
...
void OLED_CMD_init(void)
{
	GPIO_output_ctor(&oled_cmd, OLED_CMD_Port,OLED_CMD_Pin, GPIO_SET);
}
...
***************************************************************/
void OLED_CMD_control(GPIO_PIN_STATE_t value)
{
	GPIO_output_set_value(&oled_cmd,value);	
}
...
{
    SPI_CS_ctor(&oled_cs, OLED_SS_Port,OLED_SS_Pin, CS_INACTIVE, CS_ACTIVE_LOW);
}
...
***************************************************************/
void OLED_SS_control(spi_cs_state_t value)
{
    SPI_CS_set_value(&oled_cs,value);
}
...
void OLED_init(volatile SPI_t *OLED_port)
{
	...
	while(command!=0xFF)
	{
		OLED_SS_control(CS_ACTIVE); //OLED_SS_Control(0);
		SPI_transmit(OLED_port, command);
		OLED_SS_control(CS_INACTIVE); //OLED_SS_Control(1);
		index++;
