...
void STA013_output_init(gpio_inst_t * const me, volatile GPIO_t * _port_addr, uint8_t _pin_mask, GPIO_PIN_STATE_t _state)
{
    GPIO_output_ctor(me,_port_addr,_pin_mask,_state);  
}


void STA013_output_set_value(gpio_inst_t * const me, GPIO_PIN_STATE_t _state)
{
   GPIO_output_set_value(me,_state); 
}
...
uint8_t STA013_Init(void)
{
   ...
   /********* Reset the STA013 **********/
   STA013_output_init(&sta013_reset,STA013_Reset_Port,STA013_Reset_Pin,GPIO_CLEAR);
   timeout=50;
   prnt_bffr=export_print_buffer();
   STA013_output_set_value(&sta013_reset,GPIO_SET);
     ...
     for(j=0;j<18;j++)
	  {
        ...
        do
        {
            error_flag=TWI_master_transmit(STA013_TWI_Port,STA013,reg_addr,1,1,send);
            timeout--;
            if(timeout==0) LED_set_value(&led0,LED_ON);
        }while((error_flag!=no_errors)&&(timeout!=0));
