...
uint32_t find_next_clus(uint32_t Cluster_num, uint8_t array_in[])
{
   ...
   Sector=(Cluster_num>>(drive->BytesPerSecShift-drive->FATshift))+drive->StartofFAT;
   read_sector(Sector, drive->BytesPerSec,array_in);
   FAToffset=(uint16_t)((Cluster_num<<drive->FATshift)&(drive->BytesPerSec-1));
   if(drive->FATtype==FAT16)    // FAT16
   {
	   return_clus=(uint32_t)(read_value_16(FAToffset,array_in));
   ...
   {
	   return_clus=(read_value_32(FAToffset,array_in)&0x0FFFFFFF);
   }
   return return_clus;
}
