    ...
    if (error_status == NORMAL) {
       // Read the error_status value to determine what happens next (same as transmit)
        ...
        while((num_bytes!=0)&&(error_status==NORMAL)) {
            ...
            if (temp8==0x50) { // Data byte received, ACK sent
                if(int_addr_size != 0){
                    index = TWI_master_transmit_bonus(TWI_addr, int_addr, int_addr_sz, device_addr, 0, array_name);
                }
                num_bytes--;
                rec_array[index]=(TWI_addr->TWI_TWDR);
                index++;
...
uint8_t TWI_master_transmit_bonus(volatile TWI_t *TWI_addr, int32_t internal_addr, 
    ...
    */
    ...
    if (error_status == NORMAL) {
        ...
        //This changes for bonus
        // Device Address + Write bit (SLA + W)
        if(int_addr_size==0){
            send_value = slave_addr<<1;
        }
        else if(int_addr_size==1){
            send_value = internal_addr<<1;
        }        
        else if(int_addr_size==2){
            send_value = internal_addr&0xFC; //Lowest 2 bits are used
        }
        else if(int_addr_size==3){
            send_value = internal_addr&0XF8;
        }
        //sprintf(debug_buffer, "Current TWI state: 0x%02X\n\r", temp8);
        //UART_transmit_string(UART1, debug_buffer, 0);
